...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const auth = getAuth();
    let emailToLogin = idValue;

    try {
      // For cadets, we need to find their shadow email from the regimental number
      if (role === 'cadet') {
        if (!firestore) {
          throw new Error("Firestore is not available");
        }
        const usersRef = collection(firestore as Firestore, "users");
        const q = query(usersRef, where("regimentalNumber", "==", idValue));
        const querySnapshot = await getDocs(q);
        
        if (querySnapshot.empty) {
          throw new Error("Invalid Regimental Number or password.");
        }
